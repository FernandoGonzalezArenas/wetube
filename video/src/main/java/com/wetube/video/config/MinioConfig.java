    ...
    if (!isTest) {
        try {
            ...
            if (!found) {
                ...
                System.out.println("bucket de minio creado exitosamente: " + bucketName);
            }
            setPublicReadOnlyPolicy(client, "thumbnails/*");
        } catch (Exception e) {
            throw new RuntimeException("error en la creacion de el bucket de minio: " + e);
        }
    }
    return client;
}
